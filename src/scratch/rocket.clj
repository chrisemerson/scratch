...
(defn orientation
  ...
  [craft]
  (cond
    ; Initially, point along the *position* vector of the craft -- that is to say, straight up, away from the earth
    (<= (first ascent) (:time craft) (last ascent))
    (:position craft)

    ; During the circularization burn, we want to burn *sideways*, in the direction of the orbit. We'll find the
    ; component of our velocity which is aligned with our position vector (that is to say, the vertical velocity), and
    ; subtract the vertical compoent. All that's left is the *horizontal* part of our velocity.
    (<= (first circularization) (:time craft) (last circularization))
    (rejection (:velocity craft) (:position craft))

    ; Otherwis, just point straight ahead
    :else (:velocity craft)))

(defn fuel-rate
  "How fast is fuel, in kg/second, consumed by the craft?"
  [craft]
  (cond
    ; Out of fuel
    (<= (:fuel-mass craft) 0)
...
(defn stage
  "When fuel reserves are exhausted, separate stages. Otherwise, return craft changed."
  [craft]
  (cond
    ; Still fuel left
    (pos? (:fuel-mass craft))
